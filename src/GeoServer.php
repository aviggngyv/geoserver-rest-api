...
final class GeoServer {
  ...
  public function createWorkspace() {
    try {
      ...
      $this->post($route, ['workspace' => ['name' => $this->workspace]]);
    }
   ...
   *
   * @param GeoFile $file
   * @param string $charset The file charset
   ...
   */
  public function upload(GeoFile $file, $charset = 'UTF-8') {
    $store = GeoType::storeFor($file->type);
    $route = $this->routes->url("workspaces/$this->workspace/$store/$file->name/file.{$file->normalizedExtension}?charset={$charset}");
  ...
  public function uploadStyle(StyleFile $file) {
    ...
    try {
      $this->postFile($filePostRoute . '?name=' . $file->name, $file);
    }
    catch (ErrorResponseException $ex) {
  ...
  public function getRoles() {
    $res = [];
    try {
      $route = $this->routes->url("security/roles");
      $res = $this->get($route, NULL, ['Accept' => '']);
    }
    catch (ErrorResponseException $ex) {
      if ($ex->getCode() !== 401) {
        throw $ex;
  ...
  public function addRole($name) {
    try {
      $route = $this->routes->url("security/roles/role/$name");
      $this->post($route, []);
    }
    catch (ErrorResponseException $ex) {
      if ($ex->getCode() !== 401) {
   ...
   * 给用户关联角色.
   */
  public function addRoleUser($role_name, $username) {
    try {
      $route = $this->routes->url("security/roles/role/$role_name/user/$username");
      $this->post($route, []);
    }
    catch (ErrorResponseException $ex) {
      if ($ex->getCode() !== 401) {
   ...
   * 添加工作区角色权限规则.
   */
  public function addSecurityRule($workspace, $role_name) {
    try {
      $route = $this->routes->url("security/acl/layers");
      $this->post($route, [
        "$workspace.*.w" => $role_name,
        "$workspace.*.a" => $role_name,
      ]);
    }
    catch (ErrorResponseException $ex) {
      if ($ex->getCode() !== 401) {
   ...
   * 修改工作区名.
   */
  public function modifyWorkspace($old_name, $new_name) {
    try {
      $route = $this->routes->url("workspaces/$old_name");
      $this->put($route, ['workspace' => ['name' => $new_name]]);
    }
    catch (ErrorResponseException $ex) {
      if ($ex->getCode() !== 401) {
        throw $ex;
