...
trait InteractsWithHttp
{
    ...
    protected function deserialize($response, $class = null)
    {
        ...
        if (is_null($class)) {
          if ($response->hasHeader('content-type') && $response->getHeader('content-type')[0] == 'application/xml') {
    ...
    protected function get($route, $class = NULL, $header = [])
    {
        $request = $this->messageFactory->createRequest('GET', $route, $header);
